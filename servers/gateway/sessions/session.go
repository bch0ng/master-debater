...
//signingKey should be recieved from the header of the Authorization query of a post request.
//WARNING: Do not remove "Bearer" from signing key prefix.
//WARNING: signingKey IS NOT THE SAME as the sessionid sent back in newSession
//WARNING:signingKey is from context.Key NOT
func GetSessionID(r *http.Request, signingKey string) (SessionID, error) {
	...
	//validateSid, err := ValidateID(sidValue[7:], signingKey[7:])
	//validateSid, err := ValidateID(sidValue, signingKey)
	if err != nil {
		fmt.Println("Error validating sessionid in GetSessionID:",validateSid)
