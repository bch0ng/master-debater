...
func (wsc *WebsocketContext) CheckUserAuth(r *http.Request, authToken string) error {
	 ...
	 */
	sessionId, err := sessions.GetSessionID(r, wsc.Context.Key)
	if err != nil {
		fmt.Println("Error validating authtoken:", authToken, ":", err)
		return errors.New("Error validating authtoken")
	}
	newSessionState := make([]byte, 5)
	err = wsc.Context.Session.Get(sessionId, newSessionState)
	if err != nil {
		fmt.Println("Error getting sessionstate:", err)
		return errors.New("Error getting sessionState")
	}
...
func (wsc *WebsocketContext) echoSocketLoop(conn *websocket.Conn, connId int) {
	...
	for {
		...
		if msgType == CloseMessage {
			...
			err=wsc.ReplyToAllConnectionsJson(m)
			if err != nil {
